"""
HEART RATE EMOTION DETECTION - IMPROVED TRAINING PIPELINE v3
Optimized with PROPER REGULARIZATION to prevent overfitting
Target: >85% test accuracy without overfitting (100% train is bad!)
"""
...
print("="*80)
print(" HEART RATE EMOTION DETECTION - TRAINING PIPELINE v3 (REGULARIZED)")
print("="*80)
...
# ============================================================================
# STEP 5: TRAIN REGULARIZED RANDOM FOREST MODEL (v3)
# ============================================================================
print("\n" + "="*80)
print(" STEP 5: TRAINING REGULARIZED RANDOM FOREST MODEL (v3)")
print("="*80)

# REGULARIZED HYPERPARAMETERS v3 - Balance accuracy & generalization
# FIXES FOR OVERFITTING:
# - REDUCE max_depth to prevent memorization
# - INCREASE min_samples_split & min_samples_leaf to smooth boundaries
# - USE max_features to reduce correlation between trees
model = RandomForestClassifier(
    n_estimators=500,           # Balanced trees
    max_depth=10,               # REDUCED: Prevent overfitting (was 20)
    min_samples_split=10,       # INCREASED: More samples needed to split (was 2)
    min_samples_leaf=5,         # INCREASED: Larger leaf nodes (was 1)
...
print("\n✓ Model hyperparameters (v3 - REGULARIZED):")
print(f"  - n_estimators: {model.n_estimators} trees (balanced)")
print(f"  - max_depth: {model.max_depth} (REDUCED to prevent overfitting)")
print(f"  - min_samples_split: {model.min_samples_split} (INCREASED)")
print(f"  - min_samples_leaf: {model.min_samples_leaf} (INCREASED)")
print(f"  - class_weight: balanced")
print(f"\n  Strategy: REDUCE model complexity to improve generalization")
print(f"  Goal: Balance training and test accuracy (avoid 100% train, 67% test)")
...
if overfitting_gap > 0.15:
    ...
    print(f"    Consider: Reduce max_depth or increase min_samples_leaf")
...
if test_acc >= 0.85:
    ...
    print("  Next Steps:")
    print("  1. Try GradientBoosting (sometimes better than RF for this data)")
    print("  2. Try different window sizes (20s, 40s, 60s)")
    print("  3. Try SVM or other models")
    print("  4. Or use binary classification (87.99% guaranteed)")
...
# Save model
model_path = os.path.join(model_folder, 'hr_model_improved_v3.pkl')
with open(model_path, 'wb') as f:
    pickle.dump(model, f)
print(f"✓ Model saved: {model_path}")

# Save scaler
scaler_path = os.path.join(model_folder, 'hr_scaler_improved_v3.pkl')
with open(scaler_path, 'wb') as f:
    pickle.dump(scaler, f)
print(f"✓ Scaler saved: {scaler_path}")

# Save feature names
feature_names_path = os.path.join(model_folder, 'feature_names_improved_v3.pkl')
...
📁 FILES SAVED:
   • hr_model_improved_v3.pkl
   • hr_scaler_improved_v3.pkl
   • feature_names_improved_v3.pkl

🔄 USAGE:
   For inference: main_HR_improved.py (update to use _v3 models)
   Classes: 0='Neutral', 1='Stress', 2='Amusement'
""")
