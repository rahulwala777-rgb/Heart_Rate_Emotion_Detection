"""
HEART RATE EMOTION DETECTION - IMPROVED TRAINING PIPELINE v2
Optimized hyperparameters to achieve >85% accuracy on 3-class classification
"""
...
print("="*80)
print(" HEART RATE EMOTION DETECTION - TRAINING PIPELINE v2 (OPTIMIZED)")
print("="*80)
...
# ============================================================================
# STEP 5: TRAIN OPTIMIZED RANDOM FOREST MODEL (v2)
# ============================================================================
print("\n" + "="*80)
print(" STEP 5: TRAINING OPTIMIZED RANDOM FOREST MODEL (v2)")
print("="*80)

# OPTIMIZED HYPERPARAMETERS v2 - More aggressive for 3-class
model = RandomForestClassifier(
    n_estimators=800,           # INCREASED: More trees = better accuracy
    max_depth=20,               # INCREASED: Deeper trees to capture patterns
    min_samples_split=2,        # DECREASED: Allow more splits for better fit
    min_samples_leaf=1,         # DECREASED: Smaller leaf nodes
...
print("\n✓ Model hyperparameters (v2 - OPTIMIZED):")
print(f"  - n_estimators: {model.n_estimators} trees (INCREASED)")
print(f"  - max_depth: {model.max_depth} (INCREASED)")
print(f"  - min_samples_split: {model.min_samples_split} (DECREASED)")
print(f"  - min_samples_leaf: {model.min_samples_leaf} (DECREASED)")
...
if test_acc >= 0.85:
    ...
    print("  1. Try GradientBoosting instead of RandomForest")
    print("  2. Use different window sizes (20s, 40s, 60s)")
    print("  3. Try feature selection (use only top 4-5 features)")
...
# Save model
model_path = os.path.join(model_folder, 'hr_model_improved_v2.pkl')
with open(model_path, 'wb') as f:
    pickle.dump(model, f)
print(f"✓ Model saved: {model_path}")

# Save scaler
scaler_path = os.path.join(model_folder, 'hr_scaler_improved_v2.pkl')
with open(scaler_path, 'wb') as f:
    pickle.dump(scaler, f)
print(f"✓ Scaler saved: {scaler_path}")

# Save feature names
feature_names_path = os.path.join(model_folder, 'feature_names_improved_v2.pkl')
...
📁 FILES SAVED:
   • hr_model_improved_v2.pkl
   • hr_scaler_improved_v2.pkl
   • feature_names_improved_v2.pkl

🔄 USAGE:
   For inference: main_HR_improved.py (update to use _v2 models)
